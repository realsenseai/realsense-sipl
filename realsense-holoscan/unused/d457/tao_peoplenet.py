...
class PostprocessorOp(holoscan.core.Operator):
    ...
    * Make boxes compatible with Holoviz
    ...
    def compute(self, op_input, op_output, context):
        ...
        # Output tensor
        # logging.info(f"Output faces shape: {out['faces'].shape if out['faces'] is not None else 'None'}")
        # logging.info(f"Output person shape: {out['person'].shape if out['person'] is not None else 'None'}")

        # for label in out:
        #     data = out[label]
        #     logging.info(f"{label} tensor shape: {data.shape}")
        #     logging.info(f"{label} tensor content:\n{data}")

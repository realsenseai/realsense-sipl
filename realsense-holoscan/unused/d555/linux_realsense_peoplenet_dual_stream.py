...
from realsense_d555 import RealsenseCamD555
from d555_registers import RealSense_StreamId, RealSense_RGB_Mode, RealSense_Depth_Mode

import hololink as hololink_module

from realsense_tao_peoplenet import FormatInferenceInputOp, PostprocessorOp
...
class HoloscanApplication(holoscan.core.Application):
    ...
    def compose(self):
        ...
        receiver_operator_stream1 = hololink_module.operators.LinuxReceiverOperator(
            ...
            udp_port=54739 + RealSense_StreamId.DEPTH.value,
        ...
        receiver_operator_stream2 = hololink_module.operators.LinuxReceiverOperator(
            ...
            frame_size=frame_size,
            frame_context=frame_context,
            udp_port=54739 + RealSense_StreamId.RGB.value,
...
def main():
    ...
    modes_depth = RealSense_Depth_Mode
    ...
    default_configuration = os.path.join(
            os.path.dirname(__file__), "tao_peoplenet_dual_stream.yaml"
    )
    parser.add_argument(
    ...
    camera_stream_depth = RealsenseCamD555(hololink_channel_stream_depth, RealSense_StreamId.DEPTH)
    camera_stream_rgb = RealsenseCamD555(hololink_channel_stream_rgb, RealSense_StreamId.RGB)


    logging.info("camera mode stream Depth: %s", args.camera_mode_depth)
    logging.info("camera mode stream RGB: %s", args.camera_mode_rgb)

    camera_mode_depth = RealSense_Depth_Mode(
        args.camera_mode_depth
    )

    camera_mode_rgb = RealSense_RGB_Mode(

...
from dummy_csi_cam import DummyCSICameraMode, DummyCSICamera
...
class HoloscanApplication(holoscan.core.Application):
    ...
    def compose(self):
        ...
        dummy_allocator_pool = holoscan.resources.UnboundedAllocator(self)
        dummy_source = hololink_module.operators.DummyCSISourceOp(
            self,
            allocator=dummy_allocator_pool,
            name="dummy_csi_source",
            width=640,
            height=480,
        )
        ...
        #
        self.add_flow(dummy_source, csi_to_bayer_operator, {("output", "input")})
        # self.add_flow(
        #     dummy_source, image_processor_operator, {("output", "input")}
        # )
...
def main():
    ...
    assert cu_result == cuda.CUresult.CUDA_SUCCESS
    camera = DummyCSICamera()
    camera_mode = DummyCSICameraMode(0)

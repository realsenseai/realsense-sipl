 ...
 *
 * fmt formatter for NvSci color formats. (R39.2 reworked the NvSIPL config types into
 * nvsipl::sensorconfig::SensorSystemConfig; the operator no longer pretty-prints the config tree, so
 * the old config-tree formatters were dropped.) This formatter additionally handles the RAW16 Bayer16
 * formats the D457 delivers — the stock formatter throws "Unknown format" on them at TRACE verbosity.
 */

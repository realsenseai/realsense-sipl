...
env:  D457_VIEW = rgb|ir|depth   D457_PORT = 8080   D457_DIR = /tmp/live   D457_BIND = 127.0.0.1
"""
...
# Same env surface as the rest of the tooling; hardcoding 720p silently mis-decoded any other mode.
W = int(os.environ.get('D457_WIDTH', '1280'))
H = int(os.environ.get('D457_HEIGHT', '720'))
DISP = (640, 360)
DIR  = os.environ.get('D457_DIR', '/tmp/live')
VIEW = os.environ.get('D457_VIEW', 'rgb')
PORT = int(os.environ.get('D457_PORT', '8080'))
# Bind loopback by default: the stream is unauthenticated, so binding all interfaces would let
# anyone on the network watch the camera. Set D457_BIND=0.0.0.0 to opt in explicitly.
BIND = os.environ.get('D457_BIND', '127.0.0.1')
...
if __name__ == '__main__':
    ...
    for attempt in range(12):                 # tolerate a briefly-held port from a prior server
        try:
            srv = ThreadingHTTPServer((BIND, PORT), Handler); break
        except OSError as e:

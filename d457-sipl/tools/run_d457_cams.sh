...
# on the rig and launch the Holoscan player (hsb/examples/d457_sipl_player.py) showing every stream.
#
...
# D457_WIDTH/HEIGHT. See .triage/HANDOVER-d457-sipl-multilink-NEXT.md §4 for the underlying recipe
# this automates, and .triage/PLAN-d457-multicam-player.md M1c for why each step exists.
#
...
if [ "$SKIP_DEPLOY" = "0" ]; then
    ...
    # half the streams silently dead (symptom: S2,3,6,7 = 0, see HANDOVER-...-impl.md).
    ...
    "${SSH[@]}" "cd '$RIG_SIPL' && RIG_SUDO_PW='${RIG_SUDO_PW:-}' D457_MAP_LINKS=$LINKS D457_MAP_STREAMS=$STREAMS \
                 ~/build_deploy.sh -c driver,deser,query"
else
...
# script and pass just its path, same workaround as sync_hsb.sh's do_build.
PLAY_SCRIPT="$RIG_HSB/_d457_play_stage${STAGE}.sh"
{
    ...
    # `docker run --rm` discards any in-container fix, so the freshly built op .so (which sync_hsb.sh
    # just rebuilt into $RIG_HSB/build, bind-mounted) must be re-copied over the demo image's baked-in
    # dist-packages copy EVERY launch, in this SAME container invocation -- see sync_hsb.sh's do_build
...
if [ "\$running_geom" != "$VNC_GEOM" ]; then
    ...
    if [ ! -f "\$HOME/.vnc/passwd" ]; then
        mkdir -p "\$HOME/.vnc"
        echo $RIG_USER | vncpasswd -f > "\$HOME/.vnc/passwd"
...
EOF
    say "view: ssh -L ${VNC_PORT}:localhost:${VNC_PORT} $RIG_USER@$RIG_HOST, then VNC viewer -> localhost:${VNC_PORT} (password: $RIG_USER)"

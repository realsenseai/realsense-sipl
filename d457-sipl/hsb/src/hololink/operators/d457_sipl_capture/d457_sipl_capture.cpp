...
{
    // The driver reads D457_STREAM / D457_STREAMS during Init(); set the requested stream first so a
    // single-sensor config selects the right VC. (No effect on the 3-sensor config, which streams all.)
    if (!stream_.empty()) {

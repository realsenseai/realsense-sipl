 ...
 * dlsym()s `const char* CNvMQuery_GetJsonData(void)` to get the
 * camera-config JSON. This registers D457 so `nvsipl_camera -c D457_Camera` can find it.
 *
 * Schema matches the stock libnvsipl_qry_nova0_hawk.so plugin shipped with the SDK:
 * the top level is a JSON ARRAY of two objects —
 ...
 * DS5 wedge otherwise needs. The driver name "MAX20087" must match DriverInfo.name
...
[
    ...
    "cameraConfigs": [
      {
        ...
        "sensorInfo": [
          ...
          },
          {
            "name": "D457",
            "id": 2,
            "description": "RealSense D457 DS5 ASIC IR/Y (Y8I->RAW16) via DS5 mux @ 0x1A, VC2",
            "i2cAddress": "0x1a",
            "numContext": 1,
            "isTriggerModeEnabled": false,
            "deviceIndex": 2,
  ...
  },
  {
    ...
    replaceAll(json, "__W__", envNum("D457_WIDTH",  "1280", 64, 4096, true));
    replaceAll(json, "__H__", envNum("D457_HEIGHT", "720",  64, 4096, true));
    replaceAll(json, "__F__", envNum("D457_FPS",    "30.0", 1,  120,  false));
    return json;
}

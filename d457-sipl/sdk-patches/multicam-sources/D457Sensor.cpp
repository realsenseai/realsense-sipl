...
bool D457Sensor::StartStreaming(const GmslModuleContext& context)
{
    ...
    // DIAGNOSTIC (D457_L1_NOPLAY): skip link>0 owners' DS5 plays entirely — to test whether link1's
    // DS5 i2c (broadcast to link0's local bus) is what stalls link0's streaming RGB.
    if (m_link != 0U && std::getenv("D457_L1_NOPLAY") != nullptr) {
        UDDF_LOG_INFO(*context.driverServices, "D457[%u] link%u: NOPLAY diagnostic — skipping DS5 plays", m_deviceIndex, m_link);
        return true;
    }

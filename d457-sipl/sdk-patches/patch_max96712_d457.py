...
NEW_4X2_BODY = '''    def seq_set_mipi_d_phy_4x2(self):
        with Sequence('set_mipi_d_phy_4x2'):
            annotate('D457: d4xx-exact 2x4-port-mode, 4-lane on PHY0/1 (port A) @ 2500 Mbps')
            with self.max967xx:
...
def patch_mapping():
    ...
    if "D457_MAP_LINKS" in s:
        return "MAP: not applicable (multi-camera HSL generates the mapping)"
    if "write(0x0100, 0x23)" in s:
        return "MAP: already patched"
    if OLD_MAP not in s:
        # Known gap: OLD_MAP was written against an SDK whose stock mapping block differs from
        # R39.2's (which has write(0x090D, 0x2c), not 0x1e). Fail loudly -- silently skipping this

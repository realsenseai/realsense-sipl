...
Read alongside `architecture.md` (the layer map) and `.triage/FINDINGS.md` (hard-won facts). All
...
- **(a) d4xx control→register map** — `C:\work\realsense_mipi_platform_driver\kernel\realsense\d4xx.c`:
...
- **(b) DS5 firmware host control interface** —
  `C:\work\rs-fw-ds5\DS5_B0_DEV\AppServicesLib\I2cSlaveHostIf.{h,c}`: control blocks/struct
...
- **(c) Canonical master-side encoding** — `C:\work\rs-fw-ds5\DS5_B0_DEV\CoreServicesLib\D4MImager.c`:

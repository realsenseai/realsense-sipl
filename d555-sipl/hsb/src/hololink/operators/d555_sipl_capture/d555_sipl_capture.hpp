 ...
 */
class D555SIPLCaptureOp : public holoscan::Operator {
public:
    HOLOSCAN_OPERATOR_FORWARD_ARGS(D555SIPLCaptureOp)
    ...
    explicit D555SIPLCaptureOp(const std::string& camera_config, const std::string& json_config, bool raw_output, ArgsT&&... args)
...
private:
    ...
    // Mirrors D457SIPLCaptureOp: a per-camera buffer-wait timeout skips the tick by default
    // rather than throwing, so one stalled queue does not kill the application.

...
void D555SIPLCaptureOp::list_available_configs(const std::string& json_config)
{
...
void D555SIPLCaptureOp::init_cameras()
{
    if (!initialized_) {
        init_nvsipl();
        init_nvsci();
...
void D555SIPLCaptureOp::init_nvsipl()
{
    ...
    if (status != nvsipl::NVSIPL_STATUS_OK) {
        throw std::runtime_error(fmt::format(
            "Failed to initialize NvSIPLCamera (status = {})",
            static_cast<int>(status)
        ));
    }
}

void D555SIPLCaptureOp::init_nvsci()
{
    NvSciError err;
    ...
    HSB_LOG_INFO("Allocated NvSciCpuWaitContext");
}

void D555SIPLCaptureOp::fill_camera_info()
{
...
void D555SIPLCaptureOp::allocate_buffers(uint32_t camera_index, nvsipl::INvSIPLClient::ConsumerDesc::OutputType output_type, std::vector<NvSciBufObj>& bufs)
{
...
void D555SIPLCaptureOp::free_buffers(std::vector<NvSciBufObj>& bufs)
{
    for (auto buf : bufs) {
        NvSciBufObjFree(buf);
    }
...
void D555SIPLCaptureOp::allocate_sync(uint32_t camera_index, nvsipl::INvSIPLClient::ConsumerDesc::OutputType output_type, NvSciSyncObj& sync)
{
    // Helper for cleaning up attribute lists
...
void D555SIPLCaptureOp::register_autocontrol(uint32_t camera_index)
{
...
void D555SIPLCaptureOp::compute(holoscan::InputContext& op_input,
    ...
    holoscan::ExecutionContext& context)
{
    ...
    for (auto& camera_state : per_camera_state_) {
        ...
        if (status != nvsipl::NVSIPL_STATUS_OK || buffer == nullptr) {
            ...
            // Skip this tick rather than killing the application on one stalled queue; see the same
            // handling in D457SIPLCaptureOp::compute().
            HSB_LOG_WARN("Timed out waiting for buffer for {} (status = {}); skipping this tick",
                camera_state.output_name_, static_cast<int>(status));
            return;
        }
...
const std::vector<D555SIPLCaptureOp::CameraInfo>& D555SIPLCaptureOp::get_camera_info()
{
    init_cameras();
    return camera_info_;
}

bool D555SIPLCaptureOp::load_nito_file(std::string module_name, std::vector<uint8_t>& nito)
{

...
/**
 * @brief Defines SIPL @ref NvSciSync client types.
 */
...
class INvSIPLCamera
{
 public:
    ...
    /** @brief Set a platform configuration using the CameraSystemConfig structure.
     *
     * This method creates and initializes a Camera HAL instance using the provided configuration.
     * The Camera HAL will handle the matching of transports to cameras and create the appropriate
     * camera modules and groups.
     *
     * This method must be called before @ref SetPipelineCfg().
     *
     * @pre None.
     *
     * @param[in] config Camera system configuration, including all cameras and transports.
     *
     * @returns::SIPLStatus the completion status of the operation.
     *
     * @usage
     ...
     *   - De-Init: No
     */
    virtual SIPLStatus SetPlatformCfg(CameraSystemConfig const& config) = 0;
     ...
     * @returns::SIPLStatus the completion status of the operation.
     *
     * @note This function specifically returns @ref NVSIPL_STATUS_NOT_INITIALIZED along with appropriate errors
     * in the Pipeline Notification Queue that there were init errors reported from Camera Device Drivers for
     * specific pipelines
     *
     * @usage
     * - Allowed context for the API call
     ...
     * @param[in, out] imageAttr Reference to the image attributes.
     *
    ...
    virtual SIPLStatus GetImageAttributes(uint32_t const index,
                                          ...
                                          NvSciBufAttrList const &imageAttr) = 0;
     ...
     * module.
     *
     * @pre This function must be called after @ref Init() and before @ref Deinit().
     *
     ...
     *
     * @param[in] index The ID of the sensor.
     * @param[in] outType The output which images are being registered; can be ICP, ISP0, ISP1 or ISP2.
     * @param[in] images Vector of @ref NvSciBufObj to be registered.
     * @li Supported number of images that can be registered:
     *   @li [3, @ref pipelineCfg.bufferCfg.maxCaptureBufferCount] for ICP output (live capture)
     *   @li [1, @ref pipelineCfg.bufferCfg.maxCaptureBufferCount] for ICP output (reprocess mode) : Non-Safety
     *   @li [1, @ref pipelineCfg.bufferCfg.maxIsp{0/1/2}BufferCount] for ISP outputs
     * @note Total number of images supported is limited by the availability of configured memory.
     *
     ...
     * @returns::SIPLStatus the completion status of the operation.
     *
     * @note This function specifically returns @ref NVSIPL_STATUS_NOT_INITIALIZED to indicate partial init and start
     * of pipelines only for working cameras in case any issues were reported for cameras via Pipeline Notification Queue
     *
     * @usage
     * - Allowed context for the API call
     *   - Interrupt handler: No
    ...
    /** @brief Retrieve the custom interface provider for a sensor pipeline.
     *
     * Retrieve the custom interface provider for the sensor pipeline associated
     * with the specified sensor ID (@a index).
     * This allows for direct access to custom sensor pipeline functionality.
     *
     * @pre This function must be called after @ref SetPipelineCfg() and before @ref Start().
     *
     * @param[in]   index               Sensor ID associated with the sensor pipeline.
     *                                  Valid range: [ 0 .. @ref MAX_SENSORS_PER_PLATFORM - 1 ].
     * @param[out]  interfaceProvider   The custom interface provider for this sensor pipeline.
     ...
     *   - De-Init: No
     */
    virtual SIPLStatus GetPipelineInterfaceProvider(uint32_t const index,
                                                    IInterfaceProvider *&interfaceProvider) noexcept = 0;
